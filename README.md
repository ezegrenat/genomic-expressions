...
Clasificadores supervisados para predecir si una lesión pre-tumoral tiene buen o mal pronóstico, a partir de la expresión de 200 genes medidos con tecnología NGS. Comparamos seis familias de modelos con búsqueda de hiperparámetros, analizamos el trade-off sesgo-varianza de los mejores y evaluamos el modelo final sobre un conjunto held-out. El mejor modelo (SVC con kernel RBF) alcanzó un AUC-ROC de 0.9386 en datos no vistos.
...
- Comparar múltiples familias de clasificadores (Decision Tree, KNN, SVM, LDA, Naive Bayes, Random Forest) con búsqueda aleatoria de hiperparámetros.
- Diagnosticar el trade-off sesgo-varianza de los mejores modelos mediante curvas de complejidad y de aprendizaje.
...
Elegimos la proporción 80/20 como balance entre tener suficientes datos para entrenar y mantener un conjunto de control suficientemente representativo.

### Evaluación de modelos

- **Método:** K-fold cross-validation con K=5
- **Métrica principal:** AUC-ROC
- **Métricas secundarias:** Accuracy, AUPRC

### Búsqueda de hiperparámetros

Usamos `RandomizedSearchCV` con 10 iteraciones por modelo. Los hiperparámetros explorados para cada familia:

- **Árboles de decisión:** `max_depth`, `criterion`, `max_features`
- **KNN:** `n_neighbors` de 1 al 20% del train, `weights`, `metric`
- **SVM:** `kernel`, `degree`, `class_weight`, `C`
...
Evaluación final en held-out (SVC con mejores hiperparámetros): AUC-ROC = 0.9386. Predicciones guardadas en `10_y_pred_held_out_9386.csv`.

### Por qué SVC fue el mejor modelo

> El kernel RBF permite una separación más flexible entre clases en comparación con otros tipos de kernel. Dado que los atributos del dataset siguen distribuciones aproximadamente normales y no encontramos correlaciones fuertes entre features y target, SVC con kernel radial puede capturar relaciones no lineales que los demás algoritmos no modelan bien. Además, tiene menor sensibilidad a outliers que KNN.

---

## Diagnóstico sesgo-varianza (resumen)

- **Árbol de decisión:** `max_depth=4` resultó el punto de equilibrio óptimo. Profundidades mayores mejoran el AUC en train pero lo reducen en validación (sobreajuste clásico).
- **SVM:** variar C no genera cambios significativos en la varianza del modelo. Valores de C < 2 muestran subajuste; a partir de C=2 el AUC de validación se estabiliza.
...
El mejor modelo resultó ser SVC con kernel RBF (C=2.80, class_weight=balanced), alcanzando un AUC-ROC de 0.9386 en el conjunto held-out. Sospechamos que esto se debe a la naturaleza de la distribución de los datos: los features siguen un comportamiento aproximadamente normal, lo que le permite al kernel radial hacer una buena separación entre clases. A esto se suma su menor sensibilidad a outliers respecto a KNN.
...
clasificacion_expresiones_genomicas.ipynb  # notebook principal con todo el análisis                                                                   
data.csv                                   # dataset original (500 muestras × 200 features + target)                                             
desarrollo.csv                             # subconjunto de desarrollo (400 muestras, 80% de data.csv)
...
- LDA y Naive Bayes los evaluamos solo con hiperparámetros default, por lo que explorar sus espacios de búsqueda queda como trabajo pendiente.
